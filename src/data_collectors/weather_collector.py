...
class WeatherCollector:
    ...
    def get_race_weekend_weather(self, circuit_info: Dict) -> Dict:
        ...
        logger.info(f"🌤️ Getting weather forecast for {city}, {country} ({lat:.4f}, {lon:.4f})")
    ...
    def _get_current_weather(self, lat: float, lon: float) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ Current weather: {current_weather['temperature']:.1f}°C, {current_weather['weather_description']}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for item in data['list']:
                    ...
                    daily_data[date_key]['rain_probability'].append(item.get('pop', 0) * 100)
                    daily_data[date_key]['rain_volume'].append(item.get('rain', {}).get('3h', 0))
                ...
                logger.info(f"✅ Forecast: {len(forecast_data['daily_forecasts'])} days, {len(forecast_data['hourly_forecasts'])} hourly entries")
        ...
        try:
            ...
            if len(daily_forecasts) >= 3:
                ...
                analysis['race_day_conditions'] = {
                    ...
                    'weather_impact': self._assess_weather_impact(race_day, circuit_info)
                }
                ...
                analysis['strategy_implications'] = self._get_strategy_implications(race_day, circuit_info)
                
                # Weather specialists
                analysis['weather_advantage_drivers'] = self._get_weather_specialists(race_day, circuit_info)
                
                logger.info(f"✅ Weather analysis complete: {race_day['temp_avg']:.1f}°C, {race_day['rain_probability_max']:.0f}% rain")
    ...
    def _get_strategy_implications(self, race_day: Dict, circuit_info: Dict) -> List[str]:
        ...
        if rain_prob > 50:
            ...
            implications.extend([
                "Weather window strategy important",
                "Flexible pit stop plans needed"
            ])
    ...
    def _get_fallback_weather(self, circuit_info: Dict) -> Dict:
        ...
        return {
            'current_weather': {
                'available': False,
                'temperature': sum(typical_weather.get('temperature_range', [20, 25])) / 2,
            ...
            'race_weekend_analysis': {
                ...
                'fallback_conditions': {
                    'typical_temperature_range': typical_weather.get('temperature_range', [20, 25]),
                    'typical_rain_probability': typical_weather.get('rain_probability', 0.3) * 100,
