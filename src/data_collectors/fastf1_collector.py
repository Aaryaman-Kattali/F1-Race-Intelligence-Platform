...
class FastF1Collector:
    ...
    def get_circuit_historical_data(self, circuit_name: str) -> List[Dict]:
        ...
        for year in HISTORICAL_YEARS:
            try:
                ...
                if race_round:
                    logger.info(
                        f"   📅 Processing {circuit_name} {year} (Round {race_round})"
                    )

                    # Collect race weekend data
                    race_data = self._collect_race_weekend_data(
                        year, race_round, circuit_name
                    )
        ...
        logger.info(
            f"🎯 Collected {len(historical_data)} historical races for {circuit_name}"
        )
    ...
    def _find_race_round(
        self, target_circuit: str, schedule: List[Dict]
    ) -> Optional[int]:
        ...
        for event in schedule:
            ...
            if (
                target_lower in race_name
                or race_name in target_lower
                or target_lower in official_name
                or target_lower in location
            ):
                return event["round"]

        return None

    def _collect_race_weekend_data(
        self, year: int, race_round: int, circuit_name: str
    ) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(
                f"Error collecting race weekend data for {year} round {race_round}: {e}"
            )
            return None

    def _get_session_data(
        self, year: int, race_round: int, session_type: str
    ) -> Optional[Dict]:
        ...
        try:
            ...
            session_data = {
                ...
                "weather": (
                    self._extract_weather_data(session)
            ...
            if session_type == "Qualifying":
                session_data["raw_qualifying"] = self._extract_raw_qualifying(
                    session.results
                )

            # Add raw per-lap records for lap_times ingestion
            if not session.laps.empty:
                session_data["raw_laps"] = self._extract_raw_laps(
                    session.laps, session_type
                )
            ...
            quali_results.append(
                {
                    ...
                    "q1_time": self._timedelta_to_str(
                        self._safe_get(driver, "Q1", pd.NaT)
                    ),
                    "q2_time": self._timedelta_to_str(
                        self._safe_get(driver, "Q2", pd.NaT)
                    ),
                    "q3_time": self._timedelta_to_str(
                        self._safe_get(driver, "Q3", pd.NaT)
                    ),
                }
            )
    ...
    def _extract_raw_laps(self, laps: pd.DataFrame, session_type: str) -> List[Dict]:
        ...
        for _, lap in laps.iterrows():
            ...
            lap_time = self._timedelta_to_seconds(
                self._safe_get(lap, "LapTime", pd.NaT)
            )
            ...
            raw_laps.append(
                {
                    ...
                    "sector1_seconds": self._timedelta_to_seconds(
                        self._safe_get(lap, "Sector1Time", pd.NaT)
                    ),
                    "sector2_seconds": self._timedelta_to_seconds(
                        self._safe_get(lap, "Sector2Time", pd.NaT)
                    ),
                    "sector3_seconds": self._timedelta_to_seconds(
                        self._safe_get(lap, "Sector3Time", pd.NaT)
                    ),
                    "compound": (
                        self._safe_get(lap, "Compound", None)
                        if not pd.isna(self._safe_get(lap, "Compound", None))
                        else None
                    ),
                    "tyre_life": (
                        int(self._safe_get(lap, "TyreLife", 0))
                        if not pd.isna(self._safe_get(lap, "TyreLife", None))
                        else None
                    ),
                    "stint": (
                        int(self._safe_get(lap, "Stint", 0))
                        if not pd.isna(self._safe_get(lap, "Stint", None))
                        else None
                    ),
                    "position": (
                        int(self._safe_get(lap, "Position", 0))
                        if not pd.isna(self._safe_get(lap, "Position", None))
                        else None
                    ),
                    "is_personal_best": (
                        bool(self._safe_get(lap, "IsPersonalBest", False))
                        if not pd.isna(self._safe_get(lap, "IsPersonalBest", None))
                        else None
                    ),
                }
            )
    ...
    def _process_lap_data(self, laps: pd.DataFrame) -> Dict:
        ...
        lap_analysis = {"total_laps": len(laps), "drivers": {}}
    ...
    def _extract_driver_data(self, sessions: Dict) -> Dict:
        ...
        for session_name, session_data in sessions.items():
            if "laps" in session_data:
                for driver_code, lap_data in (
                    session_data["laps"].get("drivers", {}).items()
                ):
                    if driver_code in drivers:
                        drivers[driver_code][
                            f"{session_name.lower()}_analysis"
                        ] = lap_data
