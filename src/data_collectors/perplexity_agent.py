...
class PerplexityAgent:
    ...
    def _log_collected_data(self, driver_data: Dict, team_data: Dict, weekend_data: Dict):
        ...
        if driver_data.get('standings_available'):
            ...
            for i, driver in enumerate(drivers[:10], 1):  # TOP 10
                print(f"   {i:2d}. {driver.get('name', 'Unknown')} ({driver.get('code', '???')}) - {driver.get('points', 0)} pts")
            if len(drivers) > 10:
                print(f"   ... and {len(drivers)-10} more drivers")
        else:
            print("   ❌ No driver standings data available")
            print(f"   Raw response preview: {driver_data.get('raw_response', 'No response')[:200]}...")
        ...
        if team_data.get('standings_available'):
            ...
            for i, team in enumerate(teams, 1):  # ALL TEAMS
                print(f"   {i:2d}. {team.get('team', 'Unknown')} - {team.get('points', 0)} pts")
        else:
            print("   ❌ No team standings data available") 
            print(f"   Raw response preview: {team_data.get('raw_response', 'No response')[:200]}...")
        ...
        if weekend_data.get('weekend_data_available'):
            ...
            print(f"   Raw response preview: {weekend_data.get('raw_response', 'No response')[:200]}...")
    ...
    def _get_race_weekend_data(self, circuit_name: str) -> Dict:
        ...
        gp_clean = circuit_name.replace("Grand Prix", "GP").replace("Hungarian Grand Prix", "Hungarian GP")
    ...
    def _query_perplexity(self, prompt: str, query_type: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"   ✅ Successfully retrieved {query_type} data from Perplexity")
            return content
            
        except requests.exceptions.HTTPError as e:
            logger.error(f"HTTP error for {query_type}: {e.response.status_code} - {e.response.text}")
            return None
    ...
    def _parse_driver_standings(self, response: str) -> Dict:
        ...
        logger.info(f"📊 Raw driver standings response length: {len(response)} characters")
        ...
        try:
            ...
            races_match = re.search(r'after\s*(\d+)\s*races?\s*completed', response, re.IGNORECASE)
            if races_match:
                        ...
                        try:
                            ...
                            for item in match[2:]:
                                if len(item) == 3 and item.isupper():
                                    code = item
                                elif any(team_word in item.lower() for team_word in 
                                    ['red bull', 'ferrari', 'mercedes', 'mclaren', 'aston', 'alpine']):
                            ...
                            # Validate data
                            if 1 <= position <= 25 and points >= 0 and name != "Unknown":
                                drivers.append({
                                    'position': position,
                                    'name': name,
                                    'code': code,
                                ...
                                logger.debug(f"📊 Parsed: {position}. {name} ({code}) - {team} - {points} pts")
            ...
            if drivers:
                ...
                for i, driver in enumerate(drivers[:3], 1):
                    logger.info(f"📊 P{i}: {driver['name']} ({driver['code']}) - {driver['points']} pts")
        ...
        if drivers and len(drivers) >= 10:
            ...
            logger.warning(f"Only parsed {len(drivers)} drivers, using fallback with response data")
    ...
    def _parse_team_standings(self, response: str) -> Dict:
        ...
        logger.info(f"🏁 Raw team standings response length: {len(response)} characters")
        ...
        try:
            ...
            for pattern_idx, pattern in enumerate(patterns):
                ...
                if matches:
                    logger.info(f"🏁 Using team pattern {pattern_idx + 1}: found {len(matches)} matches")
            ...
            races_match = re.search(r'(\d+)\s*races?\s*completed|after\s*(\d+)\s*rounds?', response, re.IGNORECASE)
            if races_match:
                races_completed = int(races_match.group(1) or races_match.group(2))
            
            if constructors:
                constructors.sort(key=lambda x: x['position'])
                logger.info(f"🏁 Successfully parsed {len(constructors)} teams from standings")
    ...
    def _parse_race_weekend_data(self, response: str) -> Dict:
        ...
        sessions = {
            'FP1': {'available': False, 'results': [], 'fastest_driver': '', 'fastest_time': ''},
            'FP2': {'available': False, 'results': [], 'fastest_driver': '', 'fastest_time': ''},
            'FP3': {'available': False, 'results': [], 'fastest_driver': '', 'fastest_time': ''},
            'Qualifying': {'available': False, 'results': [], 'pole_position': '', 'pole_time': ''}
        }
        ...
        try:
            ...
            for pattern_idx, pattern in enumerate(quali_patterns):
                ...
                if match:
                    ...
                    logger.info(f"🏎️  Found pole position (pattern {pattern_idx + 1}): {pole_driver} - {pole_time}")
                    break
            ...
            for grid_pattern_idx, grid_pattern in enumerate(quali_grid_patterns):
                ...
                if match:
                    grid_section = match.group(1) if len(match.groups()) >= 1 else match.group(0)
                    logger.info(f"🏎️  Found qualifying section (pattern {grid_pattern_idx + 1}): {len(grid_section)} characters")
                    ...
                    position_patterns = [
                        ...
                        r'(\d+)\.\s*([A-Za-z\s]+?)(?:\s*\([^)]*\))?\s*$' if pole_time else None,
                    ]
                    ...
                    for pos_pattern_idx, pos_pattern in enumerate(position_patterns):
                        position_matches = re.findall(pos_pattern, grid_section, re.IGNORECASE | re.MULTILINE)
                        
                        if position_matches:
                            logger.info(f"🏎️  Found {len(position_matches)} qualifying positions (pattern {pos_pattern_idx + 1})")
                            ...
                            for pos_match in position_matches[:10]:  # TOP 10
                                try:
                                    ...
                                    if len(pos_match) >= 3:
                                        ...
                                        if ':' in potential_time and '.' in potential_time:
                                            time = potential_time
                                        elif len(pos_match) >= 4 and ':' in pos_match[-2]:
                                            time = pos_match[-2]
                                    ...
                                    if not time and pole_time:
                                        ...
                                        try:
                                            pole_seconds = self._time_to_seconds(pole_time)
                                            estimated_seconds = pole_seconds + (position - 1) * 0.1
                                            time = self._seconds_to_time(estimated_seconds)
                                        except:
                                            time = ""
                                    ...
                                    if 1 <= position <= 20 and driver:
                                        ...
                                        logger.debug(f"   P{position}: {driver} - {time if time else 'no time'}")
                                        
                                except (ValueError, IndexError) as e:
                                    logger.debug(f"Error parsing qualifying position {pos_match}: {e}")
                                    continue
            ...
            if pole_driver or quali_results:
                ...
                logger.info(f"🏎️  Qualifying data: {pole_driver} on pole, {len(quali_results)} grid positions")
            ...
            for session_num in ['1', '2', '3']:
                ...
                for section_pattern in practice_section_patterns:
                    ...
                    if match:
                        ...
                        logger.info(f"🏎️  Found {session_name} section: {len(practice_section)} characters")
                        ...
                        for pos_pattern in practice_position_patterns:
                            practice_matches = re.findall(pos_pattern, practice_section, re.IGNORECASE | re.MULTILINE)
                            
                            if practice_matches:
                                logger.info(f"🏎️  Found {len(practice_matches)} {session_name} results")
                                
                                for p_match in practice_matches[:10]:  # TOP 10
                                    try:
                                        position = int(p_match[0])
                                        driver = self._clean_name(p_match[1])
                                        time = p_match[2] if len(p_match) > 2 and ':' in str(p_match[2]) else ""
                                        ...
                                        logger.debug(f"Error parsing {session_name} position {p_match}: {e}")
                                        continue
                    ...
                    for pattern in individual_patterns:
                        ...
                        if match:
                            if len(match.groups()) >= 2:
                                ...
                                practice_results = [{'position': 1, 'driver': fastest_driver, 'time': fastest_time}]
                                logger.info(f"🏎️  Found {session_name} fastest: {fastest_driver} - {fastest_time}")
                                break
                ...
                if fastest_driver or practice_results:
                    ...
                    logger.info(f"🏎️  {session_name} data: {fastest_driver} fastest, {len(practice_results)} results")
            ...
            for pattern in incident_patterns:
                ...
                for match in matches:
                    incident_text = match.strip() if isinstance(match, str) else ' '.join(match).strip()
        ...
        weekend_data_available = any(session['available'] for session in sessions.values())
        ...
        if weekend_data_available:
            available_sessions = [name for name, data in sessions.items() if data['available']]
            logger.info(f"🏎️  Successfully parsed weekend data: {', '.join(available_sessions)}")
            ...
            for session_name, session_data in sessions.items():
                if session_data.get('available'):
                    ...
                    if session_name == 'Qualifying':
                        ...
                        pole_time = session_data.get('pole_time', '')
                        logger.info(f"   {session_name}: {pole} on pole ({pole_time}), {results_count} grid positions")
                    else:
                        fastest = session_data.get('fastest_driver', 'Unknown')
                        fastest_time = session_data.get('fastest_time', '')
                        logger.info(f"   {session_name}: {fastest} fastest ({fastest_time}), {results_count} results")
        else:
    ...
    def _generate_driver_code(self, name: str) -> str:
        ...
        known_codes = {
            ...
            'george russell': 'RUS', 'carlos sainz': 'SAI', 'lando norris': 'NOR',
            'oscar piastri': 'PIA', 'fernando alonso': 'ALO', 'sergio perez': 'PER',
            'lance stroll': 'STR', 'pierre gasly': 'GAS', 'esteban ocon': 'OCO',
            'alexander albon': 'ALB', 'logan sargeant': 'SAR', 'kevin magnussen': 'MAG',
            'nico hulkenberg': 'HUL', 'valtteri bottas': 'BOT', 'zhou guanyu': 'ZHO',
    ...
    def _extract_team_name(self, match_tuple) -> str:
        ...
        for item in match_tuple:
            if any(team_word in item.lower() for team_word in 
                  ['bull', 'ferrari', 'mercedes', 'mclaren', 'aston', 'alpine', 'williams', 'haas', 'sauber']):
    ...
    def _get_fallback_driver_standings(self) -> Dict:
        ...
        return {
            ...
            'drivers': [
                {'code': 'PIA', 'name': 'Oscar Piastri', 'team': 'McLaren', 'points': 266, 'position': 1},
                {'code': 'NOR', 'name': 'Lando Norris', 'team': 'McLaren', 'points': 250, 'position': 2},
                {'code': 'VER', 'name': 'Max Verstappen', 'team': 'Red Bull Racing', 'points': 185, 'position': 3},
                {'code': 'LEC', 'name': 'Charles Leclerc', 'team': 'Ferrari', 'points': 162, 'position': 4},
                {'code': 'RUS', 'name': 'George Russell', 'team': 'Mercedes', 'points': 143, 'position': 5},
                {'code': 'HAM', 'name': 'Lewis Hamilton', 'team': 'Mercedes', 'points': 109, 'position': 6},
                {'code': 'SAI', 'name': 'Carlos Sainz Jr', 'team': 'Ferrari', 'points': 83, 'position': 7},
                {'code': 'ALO', 'name': 'Fernando Alonso', 'team': 'Aston Martin', 'points': 70, 'position': 8},
                {'code': 'PER', 'name': 'Sergio Perez', 'team': 'Red Bull Racing', 'points': 60, 'position': 9},
                {'code': 'STR', 'name': 'Lance Stroll', 'team': 'Aston Martin', 'points': 35, 'position': 10},
            ],
            'races_completed': 13,
    ...
    def _get_fallback_race_weekend_data(self) -> Dict:
        ...
        return {
            ...
            'sessions': {
                ...
                'FP2': {'available': False, 'results': [], 'fastest_driver': '', 'fastest_time': ''},
                'FP3': {'available': False, 'results': [], 'fastest_driver': '', 'fastest_time': ''},
                'Qualifying': {'available': False, 'pole_position': '', 'pole_time': '', 'results': []}
            },
    ...
    def get_race_weekend_data(self, gp_name: str) -> Dict:
        ...
        try:
            ...
            if official_data and any(session.get('session_available', False) for session in official_data.values()):
    ...
    def _format_official_data(self, official_data: Dict) -> Dict:
        ...
        return {
            ...
            'sessions': {
                ...
                'Qualifying': self._format_qualifying_session(official_data.get('qualifying', {}))
            }
        }
    ...
    def _extract_qualifying_data(self, response: str, gp_name: str) -> Dict:
        ...
        try:
            ...
            for pattern in grid_patterns:
                ...
                for match in matches:
                    try:
                        ...
                        if position <= 10 and position > 1:  # Skip P1 as it's already added
                            quali_data['results'].append({
                                'position': position,
                                'driver': driver,
                                'time': time
            ...
            quali_data['results'] = sorted(quali_data['results'], key=lambda x: x['position'])
