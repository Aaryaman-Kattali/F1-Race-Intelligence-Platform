...
def ingest_circuit(
    ...
    try:
        logger.info(
            f"🏁 Ingesting {circuit_name} {'(backfill all years)' if backfill else year}"
        )
        ...
        for race in historical:
            ...
            logger.info(
                f"  📅 Processing {circuit_name} {race_year} (Round {race_round})"
            )
            ...
            if raw_quali:
                ...
                logger.warning(
                    f"  ⚠️ No qualifying data for {circuit_name} {race_year}"
                )
...
def main():
    ...
    logger.info(
        f"📦 Batch ingestion: {len(circuits)} circuits for {'all historical years' if args.backfill else args.year}"
    )
    ...
    for circuit in circuits:
        ...
        logger.info(
            f"  ✅ {circuit}: {stats['race_results']} results, "
