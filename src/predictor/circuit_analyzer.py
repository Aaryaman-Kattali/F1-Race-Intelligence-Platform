...
class CircuitAnalyzer:
    ...
    def analyze_circuit_characteristics(
        self, circuit_key: str, historical_data: List[Dict]
    ) -> Dict:
        """
        ...
        try:
            ...
            analysis = {
                ...
                "performance_indicators": self._identify_performance_indicators(
                    historical_data, circuit_config
                ),
                "success_patterns": self._analyze_success_patterns(historical_data),
                "difficulty_assessment": self._assess_circuit_difficulty(
                    circuit_config, historical_data
                ),
                "weather_impact_analysis": self._analyze_weather_patterns(
                    historical_data
                ),
                "overtaking_analysis": self._analyze_overtaking_characteristics(
                    historical_data, circuit_config
                ),
                "analysis_metadata": {
    ...
    def get_circuit_difficulty_factors(self, circuit_key: str) -> Dict:
        ...
        return {
            "overtaking_difficulty": self._rate_difficulty(
                circuit_config.get("overtaking_difficulty", "medium")
            ),
            ...
            "driver_skill_factor": self._assess_driver_skill_requirement(
                circuit_config
            ),
            "car_setup_sensitivity": self._assess_setup_sensitivity(circuit_config),
        }

    def identify_circuit_specialists(
        self, historical_data: List[Dict], min_races: int = 3
    ) -> Dict:
        """
        ...
        try:
            ...
            for team, stats in team_stats.items():
                if stats["races"] >= 3:  # Minimum 3 races
                    ...
                    if (
                        win_rate > 0.2 or podium_rate > 0.4
                    ):  # Strong performance threshold
                        team_analysis["dominant_teams"][team] = {
                            "win_rate": win_rate,
                            "podium_rate": podium_rate,
                            "avg_position": stats["avg_position"],
                            "performance_trend": self._calculate_team_trend(
    ...
    def predict_circuit_impact_factors(
        self, weather_data: Dict, circuit_key: str
    ) -> Dict:
        """
        ...
        impact_factors = {
            ...
            "temperature_effects": self._assess_temperature_effects(
                weather_data, circuit_config
            ),
            ...
            "track_evolution": self._predict_track_evolution(
                weather_data, circuit_config
            ),
            "tire_strategy_impact": self._predict_tire_strategy_impact(
                weather_data, circuit_config
            ),
            "overall_unpredictability": self._calculate_unpredictability_factor(
                weather_data, circuit_config
            ),
        }
    ...
    def _analyze_historical_performance(
        ...
        try:
            if historical_data:
                years = [
                    race.get("year") for race in historical_data if race.get("year")
                ]
                analysis["years_covered"] = sorted(years)

                # Winner diversity
                winners = [
                    race.get("race_winner")
                    for race in historical_data
                    if race.get("race_winner")
                ]
                unique_winners = set(winners)
                analysis["winner_diversity"] = (
                    len(unique_winners) / len(winners) if winners else 0
                )

                # Identify dominant periods
                analysis["dominant_periods"] = self._identify_dominant_periods(
                    historical_data
                )
    ...
    def _analyze_strategic_factors(
        ...
        strategic_factors = {
            ...
            "tire_strategy_importance": self._analyze_tire_strategy_importance(
                historical_data
            ),
            "safety_car_impact": self._analyze_safety_car_impact(historical_data),
            "drs_effectiveness": self._analyze_drs_effectiveness(
                historical_data, circuit_config
            ),
            "track_position_value": self._analyze_track_position_value(
                historical_data, circuit_config
            ),
        }
    ...
    def _identify_performance_indicators(
        self, historical_data: List[Dict], circuit_config: Dict
    ) -> Dict:
        """Identify key performance indicators for this circuit."""
        indicators = {
            "qualifying_correlation": self._calculate_qualifying_race_correlation(
                historical_data
            ),
            "practice_predictiveness": self._analyze_practice_predictiveness(
                historical_data
            ),
        ...
        try:
            ...
            if patterns["winning_starting_positions"]:
                ...
                patterns["winning_characteristics"] = {
                    ...
                    "pole_win_percentage": (
                        sum(1 for pos in positions if pos == 1) / len(positions)
                    )
                    * 100,
                    "front_row_win_percentage": (
                        sum(1 for pos in positions if pos <= 2) / len(positions)
                    )
                    * 100,
                    "top_5_win_percentage": (
                        sum(1 for pos in positions if pos <= 5) / len(positions)
                    )
                    * 100,
                }
    ...
    def _assess_circuit_difficulty(
        self, circuit_config: Dict, historical_data: List[Dict]
    ) -> Dict:
        ...
        try:
            ...
            overtaking_diff = self._rate_difficulty(
                circuit_config.get("overtaking_difficulty", "medium")
            )
            ...
            if historical_data:
                ...
                difficulty["unpredictability_factor"] = (
                    winner_diversity + upset_rate
                ) / 2
            ...
            difficulty["difficulty_factors"] = {
                ...
                "winner_unpredictability": difficulty.get(
    ...
    def _calculate_driver_statistics(self, historical_data: List[Dict]) -> Dict:
        ...
        for driver, stats in driver_stats.items():
            ...
            if stats["quali_positions"]:
                ...
                stats["quali_consistency"] = 1.0 / (
                    1.0 + np.std(stats["quali_positions"])
                )
            else:
            ...
            if stats["position_gains"]:
                ...
                stats["comeback_frequency"] = sum(
                    1 for gain in stats["position_gains"] if gain > 0
                ) / len(stats["position_gains"])
            else:
    ...
    def _assess_setup_sensitivity(self, circuit_config: Dict) -> float:
        ...
        sensitivity_count = sum(
            ...
            if any(sensitive in factor for sensitive in setup_sensitive_factors)
        )
    ...
    def _assess_temperature_effects(
        self, weather_data: Dict, circuit_config: Dict
    ) -> Dict:
    ...
    def _assess_rain_impact(self, weather_data: Dict, circuit_config: Dict) -> Dict:
        return {
            "rain_impact": (
                "high" if weather_data.get("rain_probability", 0) > 50 else "low"
            )
        }
    ...
    def _predict_track_evolution(
        self, weather_data: Dict, circuit_config: Dict
    ) -> Dict:
        return {"evolution_factor": "normal"}

    def _predict_tire_strategy_impact(
        self, weather_data: Dict, circuit_config: Dict
    ) -> Dict:
        return {"strategy_importance": "high"}

    def _calculate_unpredictability_factor(
        self, weather_data: Dict, circuit_config: Dict
    ) -> float:
        return 0.3
    ...
    def _analyze_drs_effectiveness(
        self, historical_data: List[Dict], circuit_config: Dict
    ) -> Dict:
        return {"drs_effectiveness": "medium"}

    def _analyze_track_position_value(
        self, historical_data: List[Dict], circuit_config: Dict
    ) -> Dict:
        return {
            "track_position_importance": circuit_config.get(
                "qualifying_importance", 0.7
            )
        }

    def _calculate_qualifying_race_correlation(
    ...
    def _analyze_overtaking_characteristics(
        self, historical_data: List[Dict], circuit_config: Dict
    ) -> Dict:
        ...
        overtaking_analysis = {
            ...
            "comeback_difficulty": circuit_config.get(
                "overtaking_difficulty", "medium"
            ),
            "drs_zones_effectiveness": self._rate_difficulty(
                circuit_config.get("overtaking_difficulty", "medium")
            ),
        }
        ...
        try:
            ...
            if total_winners > 0:
                overtaking_analysis["front_runner_advantage"] = (
