...
class GPPredictor:
    ...
    def predict_race_winner(self, gp_name: str, year: int = 2025) -> Dict:
        ...
        try:
            ...
            print(
                f"🏁 Type: {circuit_config.get('circuit_type', 'unknown').replace('_', ' ').title()}"
            )
            ...
            active_driver_count = sum(
                len(race.get("drivers", {})) for race in filtered_historical
            )
            print(
                f"✅ Historical data: {len(filtered_historical)} races, {active_driver_count} driver entries (2025 active only)"
            )
            ...
            print(
                f"✅ Local standings loaded: {len(current_intelligence['driver_standings']['drivers'])} drivers"
            )

            # Step 4: Weather data
            print(f"\n🌤️  Getting weather forecast...")
            weather_data = self.weather_collector.get_race_weekend_weather(
                circuit_config
            )
            ...
            if weather_data.get("current_weather", {}).get("available"):
                ...
                if race_analysis.get("race_day_conditions"):
                    race_temp = race_analysis["race_day_conditions"].get(
                        "temperature", 0
                    )
                    rain_prob = race_analysis["race_day_conditions"].get(
                        "rain_probability", 0
                    )
                    print(
                        f"🏁 Race forecast: {race_temp:.1f}°C, {rain_prob:.0f}% rain chance"
                    )
            else:
            ...
            rookies = len(
                [idx for idx in features.index if features.loc[idx, "is_rookie_2025"]]
            )
            experienced = len(features) - rookies
            print(
                f"✅ Features created: {experienced} experienced + {rookies} rookie drivers"
            )
            ...
            predictions.update(
                {
                    ...
                    "data_sources": {
                        ...
                        "weather_available": weather_data.get(
                            "current_weather", {}
                        ).get("available", False),
    ...
    def _create_local_intelligence(self, gp_name: str) -> Dict:
        ...
        try:
            ...
            if qualifying_data.get("available"):
                ...
                print(
                    f"✅ Official data retrieved - Pole: {qualifying_data.get('pole_position', 'Unknown')}"
                )
            else:
        ...
        return {
            "driver_standings": {"standings_available": True, "drivers": drivers_data},
            "team_standings": {
    ...
    def _get_constructor_standings(self) -> List[Dict]:
        ...
        constructors = [
            {"team": team, "points": points} for team, points in team_points.items()
        ]
    ...
    def _make_optimized_predictions(
        self,
        ...
        pattern_predictions = self._pattern_matching_prediction(
            features, historical_data, circuit_config
        )
        ...
        results = {
            ...
            "circuit_analysis": self._analyze_circuit_factors(
                circuit_config, weather_data
            ),
        }
        ...
        for i, (driver_code, prob) in enumerate(final_predictions.items()):
            driver_features = (
                features.loc[driver_code] if driver_code in features.index else {}
            )
        ...
        second_prob = (
            sorted_preds[1][1]["win_probability"] if len(sorted_preds) > 1 else 0
        )
        results["model_confidence"] = float(top_prob - second_prob)

        return results

    def _local_data_prediction(
        self, features: pd.DataFrame, circuit_config: Dict
    ) -> Dict:
        ...
        for driver_code in features.index:
            ...
            real_perf = CURRENT_2025_STANDINGS.get(
                driver_code, {"position": 20, "points": 0}
            )
            ...
            hist_score = (
                21 - driver_features.get("historical_avg_position", 15)
            ) / 20.0

            # Qualifying position boost (balanced)
            grid_pos = driver_features.get("grid_position", 20)
            qualifying_boost = self._calculate_qualifying_boost(
                grid_pos, qualifying_importance, driver_code
            )

            # Circuit and team advantages
            circuit_advantage = driver_features.get("circuit_type_advantage", 0.0)
            team_advantage = self._calculate_team_advantage(
                driver_features.get("current_team_2025", "Unknown")
            )
    ...
    def _calculate_qualifying_boost(
        self, grid_pos: int, quali_importance: float, driver_code: str
    ) -> float:
    ...
    def _calculate_team_advantage(self, team_name: str) -> float:
        ...
        advantage = team_performance.get(
            team_name, team_performance.get(cleaned_team, 0.01)
        )

        logger.debug(f"🏁 {team_name} team advantage: {advantage:.3f}")
        return advantage

    def _pattern_matching_prediction(
        self, features: pd.DataFrame, historical_data: List[Dict], circuit_config: Dict
    ) -> Optional[Dict]:
        """Pattern matching with active drivers."""
    ...
    def _calculate_circuit_advantage(
        self, driver_code: str, circuit_config: Dict, features: Dict
    ) -> float:
    ...
    def _combine_predictions(
        self, local_pred: Dict, pattern_pred: Optional[Dict]
    ) -> Dict:
    ...
    def _analyze_circuit_factors(
        self, circuit_config: Dict, weather_data: Dict
    ) -> Dict:
        """Circuit analysis for optimization."""
        analysis = {
            "key_success_factors": circuit_config.get("key_factors", []),
            "overtaking_difficulty": circuit_config.get(
        ...
        # Weather analysis
        race_conditions = weather_data.get("race_weekend_analysis", {}).get(
            "race_day_conditions", {}
        )
        if race_conditions:
    ...
    def _display_optimized_results(self, predictions: Dict):
        ...
        leader = next(
            (
                d
                for d in predictions["top_predictions"]
                if d["championship_position"] == 1
            ),
            {},
        )
        if leader:
        ...
        for i, pred in enumerate(predictions["top_predictions"][:10], 1):
            ...
            print(
                f"   {emoji} {pred['driver_code']} {name} ({team}) - {prob:.1f}% {rookie_flag} [P{champ_pos}]"
            )

        # Model confidence
        confidence = predictions.get("model_confidence", 0)
        confidence_level = (
            "High" if confidence > 3.0 else "Medium" if confidence > 1.5 else "Low"
        )
        print(
        ...
        print(
            f"   Historical Races (2025 drivers only): {data_sources.get('historical_races', 0)}"
        )
        ...
        print(f"\n🏁 Circuit Analysis:")
        print(
            f"   Key Factors: {', '.join(circuit_analysis.get('key_success_factors', []))}"
        )
        print(
            f"   Qualifying Importance: {circuit_analysis.get('qualifying_importance', 'Unknown')}"
        )
    ...
    def _integrate_real_qualifying_data(
        self, features: pd.DataFrame, current_intelligence: Dict
    ) -> pd.DataFrame:
