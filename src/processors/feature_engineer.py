...
class Enhanced2025FeatureEngineer:
    ...
    def __init__(self):
        ...
        logger.info(
            f"🏎️  Initialized with REAL 2025 data for {len(self.active_driver_codes)} drivers"
        )
    ...
    def _load_2025_season_data(self):
        ...
        try:
            ...
            if F1_2025_DATA_FILES["qualifying_results"].exists():
                ...
                logger.info(
                    f"✅ Loaded {len(quali_df)} qualifying results from 2025 season"
                )
            ...
            if F1_2025_DATA_FILES["sprint_results"].exists():
                ...
                logger.info(
                    f"✅ Loaded {len(sprint_df)} sprint results from 2025 season"
                )
            ...
            if F1_2025_DATA_FILES["sprint_qualifying"].exists():
                ...
                logger.info(
                    f"✅ Loaded {len(sprint_quali_df)} sprint qualifying results from 2025 season"
                )
    ...
    def create_prediction_features(
        ...
        try:
            ...
            filtered_historical_data = self._filter_historical_to_active_drivers(
                historical_data
            )
            ...
            experienced_drivers = len(
                [f for f in features_data if not f["is_rookie_2025"]]
            )
            rookie_drivers = len([f for f in features_data if f["is_rookie_2025"]])

            logger.info(
                f"✅ Created {len(features_df)} driver features with REAL 2025 data"
            )
            logger.info(
                f"   - {experienced_drivers} experienced drivers (historical + 2025 data)"
            )
    ...
    def _filter_historical_to_active_drivers(
    ...
    def _get_teammate(
        self, driver_code: str, historical_data: List[Dict], current_intelligence: Dict
    ) -> Optional[str]:
        """
                ...
                if driver_code in drivers_in_race:
                    ...
                    if team:
                        ...
                        for other_code, other_data in drivers_in_race.items():
                            if (
                                other_code != driver_code
                                and other_data.get("team") == team
                            ):
                                return other_code

        # 3. Last resort fallback
        driver_info = next(
            (d for d in ACTIVE_DRIVERS_2025 if d["code"] == driver_code), None
        )
        if driver_info:
            ...
            return next(
                (
                    d["code"]
                    for d in ACTIVE_DRIVERS_2025
                    if d["team"] == team and d["code"] != driver_code
                ),
                None,
            )
    ...
    def _create_driver_features_real_2025(
        self,
        driver_code: str,
        historical_data: List[Dict],
        current_intelligence: Dict,
        ...
        try:
            ...
            total_races = sum(
                1 for race in historical_data if driver_code in race.get("drivers", {})
            )
            ...
            if is_rookie or data_confidence < 0.4:
                logger.debug(
                    f"🆕 Low confidence ({data_confidence}) for {driver_code}: using teammate proxy"
                )
                teammate = self._get_teammate(
                    driver_code, historical_data, current_intelligence
                )

                # Fetch teammate's historical features as a baseline if available
                if teammate:
                    hist_features = self._create_historical_features(
                        teammate, historical_data, circuit_config
                    )
                    # Apply a small "rookie/inexperience tax" to the teammate proxy
                    hist_features["historical_avg_position"] = min(
                        20, hist_features.get("historical_avg_position", 10) + 2
                    )
                    hist_features["historical_consistency"] = max(
                        0.1, hist_features.get("historical_consistency", 0.5) * 0.8
                    )
                else:
                    hist_features = self._create_rookie_features_from_2025_data(
                        driver_code
                    )
            else:
                logger.debug(
                    f"📈 Experienced driver {driver_code} (confidence: {data_confidence})"
                )
                hist_features = self._create_historical_features(
                    driver_code, historical_data, circuit_config
                )
            ...
            weather_features = self._create_weather_features(
                driver_code, weather_data, circuit_config
            )
            features.update(weather_features)

            # Circuit-specific features
            circuit_features = self._create_circuit_features(
                driver_code, circuit_config, historical_data
            )
    ...
    def _create_rookie_features_from_2025_data(self, driver_code: str) -> Dict:
        ...
        if "races" in self.season_2025_data:
            driver_races = self.season_2025_data["races"][
                self.season_2025_data["races"]["Driver"].str.contains(
                    driver_code, na=False
                )
                | self.season_2025_data["races"]["No"].astype(str)
                == str(self._get_driver_number(driver_code))
            ]

            for _, race in driver_races.iterrows():
                if (
                    pd.notna(race["Position"])
                    and race["Position"] != "NC"
                    and race["Position"] != "DQ"
                ):
                    try:
        ...
        if race_finishes:
            ...
            points_rate = len([f for f in race_finishes if f <= 10]) / len(
                race_finishes
            )
        ...
        logger.debug(
            f"🆕 Rookie {driver_code}: Real 2025 avg finish P{avg_finish:.1f}, {current_points} pts"
        )
        return features
    ...
    def _create_real_2025_form_features(self, driver_code: str) -> Dict:
        ...
        logger.debug(
            f"Real 2025 form for {driver_code}: P{position}, {points} pts, form {form_score:.2f}"
        )
        return features

    def _create_qualifying_features_real_2025(
        self, driver_code: str, current_intelligence: Dict, circuit_config: Dict
    ) -> Dict:
        ...
        if "qualifying" in self.season_2025_data:
            driver_quali = self.season_2025_data["qualifying"][
                self.season_2025_data["qualifying"]["Driver"].str.contains(
                    driver_code, na=False
                )
                | self.season_2025_data["qualifying"]["No"].astype(str)
                == str(self._get_driver_number(driver_code))
            ]
            ...
            for _, quali in driver_quali.iterrows():
                if (
                    pd.notna(quali["Position"])
                    and quali["Position"] != "NC"
                    and quali["Position"] != "DQ"
                ):
                    try:
    ...
    def _create_historical_features(
        self, driver_code: str, historical_data: List[Dict], circuit_config: Dict
    ) -> Dict:
            ...
            features.update(
                {
                    ...
                    "historical_win_rate": sum(1 for pos in race_positions if pos == 1)
                    / len(race_positions),
                    "historical_podium_rate": sum(
                        1 for pos in race_positions if pos <= 3
                    )
                    / len(race_positions),
                    "historical_points_rate": sum(
                        1 for pos in race_positions if pos <= 10
                    )
                    / len(race_positions),
                }
            )
        ...
        if quali_positions:
            ...
            features.update(
                {"historical_quali_avg": 12.0, "historical_quali_best": 15.0}
            )
    ...
    def _create_weather_features(
        self, driver_code: str, weather_data: Dict, circuit_config: Dict
    ) -> Dict:
    ...
    def _create_circuit_features(
        self, driver_code: str, circuit_config: Dict, historical_data: List[Dict]
    ) -> Dict:
        ...
        if (
            overtaking_difficulty in ["high", "very_high"]
            and driver_code in overtaking_masters
        ):
            features["overtaking_advantage"] = (
                0.15 if not is_rookie_driver_2025(driver_code) else 0.05
            )
        else:
    ...
    def _create_expected_performance_features(
        self, driver_code: str, features: Dict, circuit_config: Dict, is_rookie: bool
    ) -> Dict:
        ...
        expected_race = expected_features[
            "expected_quali_position"
        ] * quali_importance + features.get("historical_avg_position", 15) * (
            1 - quali_importance
        )
        ...
        if is_rookie:
            ...
            expected_race = expected_race + (
                rookie_penalty * 3
            )  # Smaller penalty with real data

        expected_features["expected_race_position"] = min(20, max(1, expected_race))

        # Competitiveness score
        competitiveness = (
            features.get("current_form_score", 0.5)
            * 0.4  # Increased weight for 2025 form
    ...
    def _create_real_2025_form_features(self, driver_code: str) -> Dict:
        ...
        driver_perf = CURRENT_2025_STANDINGS.get(
            driver_code, {"position": 20, "points": 0, "team": "Unknown"}
        )
        ...
        logger.debug(
            f"LOCAL 2025 data for {driver_code}: P{position}, {points} pts, form {form_score:.2f}"
        )
        return features
