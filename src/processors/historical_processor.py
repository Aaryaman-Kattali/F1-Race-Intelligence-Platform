...
class HistoricalProcessor:
    ...
    def analyze_historical_patterns(self, historical_data: List[Dict], circuit_config: Dict) -> Dict:
        """
        ...
        try:
            analysis = {
                ...
                'qualifying_impact': self._analyze_qualifying_impact(historical_data, circuit_config),
                ...
                'driver_specialists': self._identify_circuit_specialists(historical_data),
                'overtaking_analysis': self._analyze_overtaking_patterns(historical_data),
                'strategic_insights': self._analyze_strategic_patterns(historical_data),
                'reliability_factors': self._analyze_reliability_patterns(historical_data)
            }
            ...
            for race in historical_data:
                ...
                if winner and year:
                    ...
                    if driver_data:
                        ...
                        if 'qualifying_positions' not in winner_analysis['winning_characteristics']:
                            winner_analysis['winning_characteristics']['qualifying_positions'] = []
                        
                        if quali_pos != 99:
                            winner_analysis['winning_characteristics']['qualifying_positions'].append(quali_pos)
            
            # Calculate statistics
            quali_positions = winner_analysis['winning_characteristics'].get('qualifying_positions', [])
            if quali_positions:
                winner_analysis['winning_characteristics']['avg_winning_quali_pos'] = np.mean(quali_positions)
                winner_analysis['winning_characteristics']['pole_win_rate'] = sum(1 for pos in quali_positions if pos == 1) / len(quali_positions)
                winner_analysis['winning_characteristics']['front_row_win_rate'] = sum(1 for pos in quali_positions if pos <= 2) / len(quali_positions)
    ...
    def _analyze_qualifying_impact(self, historical_data: List[Dict], circuit_config: Dict) -> Dict:
        ...
        try:
            ...
            if total_races > 0:
                ...
                if len(quali_positions) > 1:
                    ...
                    quali_analysis['position_correlation'] = correlation if not np.isnan(correlation) else 0.0
                
                # Calculate qualifying importance score
                circuit_quali_importance = circuit_config.get('qualifying_importance', 0.7)
                historical_importance = (quali_analysis['pole_to_win_rate'] + quali_analysis['front_row_advantage']) / 2
                quali_analysis['qualifying_importance_score'] = (circuit_quali_importance + historical_importance) / 2
        ...
        try:
            ...
            for race in historical_data:
                ...
                if weather_data.get('available'):
                    ...
                    temp_category = 'hot' if temp > 30 else 'cold' if temp < 15 else 'moderate'
    ...
    def _analyze_team_performance(self, historical_data: List[Dict]) -> Dict:
        ...
        try:
            ...
            for team, results in team_results.items():
                ...
                points_finishes = sum(1 for result in results if result['position'] <= 10)
                ...
                positions = [result['position'] for result in results if result['position'] != 99]
                if positions:
        ...
        try:
            ...
            for race in historical_data:
                ...
                for driver_code, driver_data in drivers.items():
                    ...
                    if race_pos != 99:
                        driver_performance[driver_code]['race_positions'].append(race_pos)
                    if quali_pos != 99:
                        driver_performance[driver_code]['quali_positions'].append(quali_pos)
    ...
    def _analyze_overtaking_patterns(self, historical_data: List[Dict]) -> Dict:
        ...
        try:
            ...
            if total_position_changes:
                overtaking_analysis['average_position_changes'] = np.mean(total_position_changes)
            
            if total_winners > 0:
                overtaking_analysis['front_runner_dominance'] = front_runner_wins / total_winners
        ...
        try:
            ...
            for race in historical_data:
                ...
                for driver_code, driver_data in drivers.items():
                    ...
                    if status != 'Finished' and driver_data.get('race_position', 1) == 99:
            ...
            if total_entries:
                ...
                reliability_analysis['reliability_impact'] = total_dnfs / total_races if total_races > 0 else 0.0
