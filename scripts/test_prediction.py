...
def main():
    ...
    parser.add_argument(
        "--test-year", type=int, default=2024, help="Year to test against"
    )
    parser.add_argument(
        "--actual-winner", required=True, help="Actual race winner (3-letter code)"
    )
    ...
    try:
        ...
        print(f"\n📈 ACCURACY METRICS:")
        print(
            f"   Winner Predicted: {'✅ Yes' if accuracy.get('winner_predicted') else '❌ No'}"
        )
        print(
            f"   Top 3 Accuracy: {'✅ Yes' if accuracy.get('top_3_accuracy') else '❌ No'}"
        )
        print(
            f"   Top 5 Accuracy: {'✅ Yes' if accuracy.get('top_5_accuracy') else '❌ No'}"
        )
