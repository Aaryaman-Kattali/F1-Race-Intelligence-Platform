...
def main():
    ...
    for table in tables_to_migrate:
        ...
        try:
            ...
            client.query(
                f"CREATE OR REPLACE TABLE `{temp_table_id}` AS SELECT * FROM `{table_id}`"
            ).result()
    ...
    for table in tables_to_migrate:
        ...
        try:
            ...
            if partitioning:
                logger.info(
                    f"  - Partitioning: {partitioning.field} (Range: {partitioning.range_.start} to {partitioning.range_.end})"
                )
            else:
